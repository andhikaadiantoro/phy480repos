...
main (void)
{
  ...
  ofstream sampk10;
  sampk10.open ("sampling_test_kT10.dat");
  sampk10 << "#  energy     exact      random      metropolis  " << endl;
  for (int i = 0; i < num_energies; i += 4)  // Note the += 4 here!
  {
    sampk10 << fixed << "  " << setw(5) << i - num_sites << "  "
  ...
  double sum_met = 0;
  double avg;
  for (int i = 0; i <= num_energies; i +=4) {
        sum_met += dist_metropolis[i];
        avg = sum_met/num_energies;
        cout << avg;
    }
