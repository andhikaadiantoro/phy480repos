...
main ()
{
  ...
  double b_ho = 30;			// ho length parameter 
...
//   cin >> b_ho;
  ...
  //ofstream errorout("basis.dat");
  for(int k = 1; k <= 20; k++){

      dimension = k;

      ofstream errorout("basis20.dat");
    ...
    for (int i = 0; i < dimension; i++)
        ...
        for (int j = 0; j < dimension; j++)
        {
          errorout << scientific << gsl_vector_get (eigenvector_ptr, j) << endl;
        }
        }
    ...
    //    ho_parameters.i = i; 
    //    gsl_vector_alloc (dimension)*i;

    // }
    // errorout << log10(dimension) << " " << log10(fabs((gsl_vector_get(Eigval_ptr,0) + 0.5)/0.5)) <<" "<<gsl_vector_get(Eigval_ptr,0)<< endl; 
