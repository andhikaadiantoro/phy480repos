...
main ()
{
  ...
  double f_ext = 10.; //originally 0
  double omega_ext = 3.14; //originally 1
  double phi_ext = 0.;

  double m = 1.;
  double k = pow (2. * pi, 2);
  double p = 2.; //originally 2
  double x0 = 0.;               // initial position
  double v0 = 1.;               // initial velocity
  double b = 8;   //damping parameter

  double h = 0.001;		// initialize mesh spacing // originally 0.001
  ...
  if (i == 1)  // second equation
    {
      if (x == 0)
	{
	  return ((F_ext-b*y[1]) / m); //indicates acceleration
	}
      else if (x < 0)
	{
	  return ((F_ext-b*y[1]+ k * pow (fabs (x), (p - 1))) / m);
	}
      else if (x > 0)
	{
	  return ((F_ext-b*y[1] - k * pow (fabs (x), (p - 1))) / m);
