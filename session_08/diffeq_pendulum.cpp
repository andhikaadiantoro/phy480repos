...
main (void)
{
  ...
  double f_ext = 0.9; // originally 0.2
  double omega_ext = 0.52; //originally 0.689
  double T_ext = 2. * pi / omega_ext;  // period for external frequency
  double phi_ext = 0.;
  double omega0 = 1.;
  double alpha = 0.2; // originally 0.2
  double theta0 = -0.8;    // initial (angular) position //originally 0.8
  double theta_dot0 = 0.1234;      // initial (angular) velocity 

  int T_skip = 1000;    // every T_skip points means once every T_ext
  double h = T_ext / double(T_skip);  // initialize mesh spacing 
  double tmin = 0.1;      // starting t value //originally 0
  ...
  while (answer2 != 0)    // iterate until told to move on 
  {
    ...
    while (answer != 0)  // iterate until told to move on 
    {
      ...
      cout << "[2] alpha = " << fixed << setprecision(2) << alpha << endl;
      cout << " [3] f_ext = " << f_ext << "\t";
